...
from keras.layers import Dense, Dropout, Activation, Input, LSTM, Embedding, Concatenate
...
class emb_AttentionLayer(Layer):
    ...
    def call(self, x, mask=None):
        ...
        eij_0 = K.reshape(eij_0, (K.shape(x[0])[0], K.shape(x[0])[1]))
        #print("eij_0", K.int_shape(eij_0))

        ai_0 = K.exp(eij_0)
        #weights_0=ai_0 / K.sum(ai_0, axis=1).dimshuffle(0, 'x')
        #weights_0 = ai_0 / K.sum(ai_0, axis=1).dimshuffle(0, 'x')
        weights_0 = ai_0 / K.expand_dims(K.sum(ai_0, axis=1), -1)
        #print("weights_0", K.int_shape(weights_0))

        eij_1 = K.batch_dot(sentence, entity_1, axes=[2, 2])
        eij_1 /= 2000
        #eij_1 = K.reshape(eij_1, (x[0].shape[0], x[0].shape[1]))
        eij_1 = K.reshape(eij_1, (K.shape(x[0])[0], K.shape(x[0])[1]))
        #print("eij_1", K.int_shape(eij_1))

        ai_1 = K.exp(eij_1)
        #weights_1 = ai_1 / K.sum(ai_1, axis=1).dimshuffle(0, 'x')
        weights_1 = ai_1 / K.expand_dims(K.sum(ai_1, axis=1), -1)
        #print("weights_1", K.int_shape(weights_1))

        weights=((weights_0+weights_1)/2.0)*self.scaler

        #weighted_input = x[3] * weights.dimshuffle(0, 1, 'x')
        #weighted_input = x[3] *  K.expand_dims(K.permute_dimensions(weights, [0, 1]), 1)
        weighted_input = x[3] * K.expand_dims(weights, -1)
        print("weighted_input", K.int_shape(weighted_input))
...
if __name__ == '__main__':
        ...
        print([x1.get_shape().as_list() for x1 in [word_fea_0, pos_fea_0, dis_fea1_0, dis_fea2_0]])
        emb_merge_0 = Concatenate()([word_fea_0, pos_fea_0, dis_fea1_0, dis_fea2_0])
        print("emb_merge_0", emb_merge_0.get_shape().as_list())
        emb_merge_1 = Concatenate()([word_fea_1, pos_fea_1, dis_fea1_1, dis_fea2_1])
        emb_merge_2 = Concatenate()([word_fea_2, pos_fea_2, dis_fea1_2, dis_fea2_2])
        ...
        emb_merge_shortest = Concatenate()([shortest_word_fea, shortest_pos_fea,shortest_dis_fea1,shortest_dis_fea2])
        ...
        print("emb_merge_0", emb_merge_0.get_shape().as_list())
        #bottom RNNs
        left_gru_0 = LSTM(units=s['first_hidden_layer'],
                          kernel_initializer='orthogonal',
                          activation='tanh',
                          recurrent_activation='sigmoid')(emb_merge_0)

        right_gru_0 = LSTM(units=s['first_hidden_layer'],
                           kernel_initializer='orthogonal',
                        activation='tanh',
        ...
        left_gru_1 = LSTM(units=s['first_hidden_layer'],
                          kernel_initializer='orthogonal',
                         activation='tanh',
                         recurrent_activation='sigmoid')(emb_merge_1)

        right_gru_1 = LSTM(units=s['first_hidden_layer'],
                           kernel_initializer='orthogonal',
                          activation='tanh',
                          recurrent_activation='sigmoid',
                          go_backwards=True)(emb_merge_1)

        gru_merge_1 = Concatenate()([left_gru_1, right_gru_1])

        left_gru_2 = LSTM(units=s['first_hidden_layer'],
                          kernel_initializer='orthogonal',
                         activation='tanh',
                         recurrent_activation='sigmoid')(emb_merge_2)

        right_gru_2 = LSTM(units=s['first_hidden_layer'],
                           kernel_initializer='orthogonal',
                          activation='tanh',
                          recurrent_activation='sigmoid',
                          go_backwards=True)(emb_merge_2)

        gru_merge_2 = Concatenate()([left_gru_2, right_gru_2])

        left_gru_shortest = LSTM(units=s['first_hidden_layer'],
                                 kernel_initializer='orthogonal',
                         activation='tanh',
                         recurrent_activation='sigmoid')(emb_merge_shortest)

        right_gru_shortest = LSTM(units=s['first_hidden_layer'],
                                  kernel_initializer='orthogonal',
                          activation='tanh', 
                          recurrent_activation='sigmoid',
                          go_backwards=True)(emb_merge_shortest)

        gru_merge_shortest = Concatenate()([left_gru_shortest, right_gru_shortest])
        ...
        gru_merge_temp = Concatenate(axis=1)([gru_merge_0, entity_fea_0,gru_merge_1,gru_merge_shortest,
                                      entity_fea_1,gru_merge_2])

        #top RNNs
        left_gru_top = LSTM(units=s['first_hidden_layer'],
                            kernel_initializer='orthogonal',
                                # dropout_W=0.5, dropout_U=0.5,
                                activation='tanh',  # return_sequences=True,
                                recurrent_activation='sigmoid')(gru_merge_temp)

        right_gru_top = LSTM(units=s['first_hidden_layer'],
                             kernel_initializer='orthogonal',
                                 # dropout_W=0.5, dropout_U=0.5,
                                 activation='tanh',  # return_sequences=True,
                                 recurrent_activation='sigmoid',
        ...
        gru_merge_top = Concatenate()([left_gru_top, right_gru_top])
