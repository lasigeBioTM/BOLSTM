...
def exit_handler():
    ...
    global synonym_match
    global chebi_cache
    print("multiple_match_count", multiple_match_count)
    print("perfect_match", perfect_match)
    print("synonym_match", synonym_match)
...
def map_to_chebi(text, name_to_id, synonym_to_id):
    ...
    if text in chebi_cache:
        ...
        if text in name_to_id:
            chebi_cache[text] = text
            perfect_match += 1
            return text, used_syn
        if text in synonym_to_id:
            synonym_match += 1
            chebi_cache[text] = text
            perfect_match += 1
            return text, used_syn
        drug = process.extract(text, name_to_id.keys(), scorer=fuzz.token_sort_ratio, limit=2)
        if len(drug) > 1:
